---
name: commit-pr
description: Divide all uncommitted branch changes into logical, layered commits following the project's commit conventions (`.claude/rules/commit-conventions.md`). Presents a plan for user approval before making any commits.
...
Before starting, **read `CLAUDE.md`** in the project root for the main branch name and build artifacts that must not be committed, and **read the project's commit conventions** (`.claude/rules/commit-conventions.md`) for commit grouping order and message format.
...
Extract the ticket identifier from the branch name using the pattern described in the project's commit conventions (`.claude/rules/commit-conventions.md`) (e.g. `feature/PROJ-123-description` → `PROJ-123`). If the branch name contains no recognisable identifier, note this and use a placeholder.
...
**Use the commit grouping order from the project's commit conventions** (`.claude/rules/commit-conventions.md`) to determine the sequencing. Commits should be ordered so each builds on the previous.
...
- Follow any layer-separation rules from the project's commit conventions (`.claude/rules/commit-conventions.md`) (e.g. do not mix backend and frontend source in the same commit)
...
3. Commit using the approved message and the commit message format from the project's commit conventions (`.claude/rules/commit-conventions.md`):
...
## Step 5: Update CLAUDE.md and project rules

After all commits have been created successfully, review what was added and update `CLAUDE.md` or `.claude/rules/` files if any of the committed changes affect how Claude Code should understand or work with this project.

### What warrants an update

Read `CLAUDE.md` and the relevant `.claude/rules/` files, then check the committed files against these categories:

- **New top-level module/package** — add it to the module/package inventory in CLAUDE.md
- **New entry point** (widget, CLI command, API endpoint) — add it to the relevant architecture section in CLAUDE.md
- **New key dependency** (not dev tooling) — add to the dependencies section in CLAUDE.md
- **New convention or pattern** introduced (e.g. a new data-passing mechanism between layers) — add a note in the appropriate `.claude/rules/` file
- **New reuse reference** (a feature that should be the go-to example for future similar work) — add to the Reuse Before Reimplementing table in the appropriate `.claude/rules/` file
- **Pure implementation files** (components, resolvers, templates) — no update needed

### How to update

1. Read `CLAUDE.md` and relevant `.claude/rules/` files to understand the current structure
...
Follow the commit message format specified in the project's commit conventions (`.claude/rules/commit-conventions.md`). The general principles are:
